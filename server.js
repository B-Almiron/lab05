...
// 
function guardarmensaje(chat, mensaje) {
    let mensajes = [];
    try {
    ...
    mensajes.push({ chat: chat, mensaje: mensaje });
    const json = JSON.stringify(mensajes);
    fs.writeFileSync('mensajes.json', json, 'utf8');
}
